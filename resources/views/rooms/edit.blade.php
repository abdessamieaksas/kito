<x-dashboard-layout>
    <div class="p-6 max-w-4xl mx-auto bg-white rounded-lg shadow-md">
        <div class="flex justify-between items-center mb-6">
            <h2 class="text-2xl font-semibold text-gray-800">Edit Room: {{ $room->room_number }}</h2>
        ...
        <form action="{{ route('rooms.update', $room) }}" method="POST" enctype="multipart/form-data" class="space-y-6">
            ...
            <div>
                ...
                <img src="{{ $room->image_url }}" alt="Room {{ $room->room_number }}" class="h-48 w-auto object-cover rounded-lg border border-gray-200">
            </div>
            @endif
            
            <!-- Room Image -->
            <div>
                <label class="block text-sm font-medium text-gray-700 mb-2">{{ $room->image_path ? 'Change Image' : 'Add Image' }}</label>
